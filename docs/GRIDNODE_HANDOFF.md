...
## ⏰ SYNC NOTE (2026-06-25 19:23 UTC)

**Local sandbox synced to rc29.0 (the version on live).**

- Live SHA: `b12e31fc14194172dcfbabaf1a100454439c92449fa73dcecaf2b623b5fae0b7`
- Local baseline SHA: `b12e31fc14194172dcfbabaf1a100454439c92449fa73dcecaf2b623b5fae0b7`
- Size: 1,031,931 bytes (1.0 MB)
- Pre-sync baseline (rc27): `0071104fc012fba3414295be3ce915d1247a36d83c62cbecee4b5ed2f0f6d895` (backed up)
- SW cache: `gridnode-v1-pwa-v13`
- Cloud layer: live (Supabase adapter, magic link + password auth)
- OAuth: hidden, ready to enable

The +46 KB delta from baseline includes the cloud adapter (~18 KB), brand UI rewrite (~10 KB), and hydration code (~16 KB).

---

## 🚀 RC30.0 (2026-06-25 20:19 UTC) — Ponytail Chunk Extraction

**Status:** SHIPPED ✅
**Live SHA:** `bf75d3eb676143c9a7c7e201a0af6322898a2a1814bd4c98bbf267d2d90186f4`
**Size:** 932,294 bytes (1.03 MB → 0.93 MB, **-9.7%**)
**SW:** v14
**GitHub:** commit `cf72a91b`

### What changed

- **Scanner images extracted** — 73KB base64 JPEG → real file `scanner-body.jpeg`. Saves ~98KB inline. Cacheable by browser. Offline fallback (data-back attr) preserved.
- **Icons extracted** — 4.8KB + 47KB inline PNGs → real files `icon-192.png`, `icon-512.png`. Real PNGs served with `image/png` MIME type.
- **manifest.json extracted** — 852 bytes as separate file. Now served as `application/manifest+json` via `_headers`.
- **`_headers` file added** — CF Pages config for proper MIME types on manifest/icons/scanner-body. Immutable cache headers on static assets.
- **GN_CLOUD auto-enable bug FIXED** — Was flipping flag to '1' on sign-in regardless of user choice. Now respects explicit user choice (set on sign-in form).
- **Dead code removed** — 1.9KB of unreferenced functions removed (kept onclick-referenced ones).
- **Cloud adapter extracted** — 32.9KB Sc/GN/signIn* code moved to `/workspace/gridnode-project/_src/cloud-adapter.js` for future modularity. Reference file only — not yet wired into deploy.

### Why this matters

- **Sub-agents can now work** — file under 1MB means sub-agents don't abort on real tasks
- **PWA install now works** — proper MIME types + manifest + icons = installable app
- **Faster mobile load** — images cached separately, only re-downloaded when changed
- **Future-proofed** — modular reference ready when you want to fragment properly

### Preserved

- All 4 protected systems (scanner / shot data / weight / dosing)
- All 130+ protected keywords
- Cloud layer (magic link + password)
- OAuth (still hidden, ready to enable)
- All 44 script blocks
- All 12 prior deploys' worth of fixes

### Backup

`gridnode-GOOD-2026-06-25_pre-rc30-live-rc29.html` (previous live version, 1,031,931 bytes)

### Files in deploy

| File | Size | Purpose |
|---|---|---|
| `_headers` | 602 B | CF Pages MIME type config |
| `icon-192.png` | 4.8 KB | PWA icon (small) |
| `icon-512.png` | 47 KB | PWA icon (large) |
| `index.html` | 932 KB | Main app |
| `manifest.json` | 852 B | PWA manifest |
| `scanner-body.jpeg` | 73 KB | Scanner reference image |

### Next

- Test PWA install on Android Chrome + Firefox
- Verify cloud sync flow (sign in → log shot → open Firefox → see shot)
- Decide if/when to wire `cloud-adapter.js` into deploy as `<script src="cloud-adapter.js">`
...
**11 deploys shipped today. Live product is v1.3.1-rc27. Read this section first if you just started.**

### ✅ What got shipped (rc14 → rc27)
...
- ✅ **DASH empty-state CTA** - Replaced "TAP THE + BUTTON" text with real `<button class="btn-full btn-primary empty-cta" onclick="handleShotFab()">LOG YOUR FIRST SHOT</button>`. Mobile tap target meets 44px. aria-label included. (rc27)

### ✅ Current locked state (rc27, 2026-06-22)
...
4. ✅ Read `/workspace/.gridnode-handoff/MAVIN-TIPS-TRICKS-TACTICS.md` (25 lessons from 14h session)
5. ✅ Read `/workspace/.gridnode-handoff/MAVIN-AND-CLAUDE-COLLAB.md` (multi-agent patterns)
6. ✅ Read `/workspace/.gridnode-handoff/CREDENTIALS.md` (credential policy)
7. ✅ Ask Pipe: "What first?" — likely options: copy rewrite, ship 5 visual fixes, Supabase signup
...
- **He works late into the evening** — observed firsthand (June 22 session ran 14+ hours ending at 11:57 PM Eastern). He doesn't rush, even when tired. He asks "anything else?" when the work is essentially done — that's his tell that the session is winding down.
